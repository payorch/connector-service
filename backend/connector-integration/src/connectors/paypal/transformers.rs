...
impl TryFrom<&PaymentsAuthorizeData> for PaypalPaymentRequest {
    type Error = error_stack::Report<errors::ConnectorError>;
    fn try_from(data: &PaymentsAuthorizeData) -> Result<Self, Self::Error> {
        // Paypal expects string amount (major units, 2 decimal places)
        let amount = format!("{:.2}", (data.amount as f64) / 100.0);
        let currency = data.currency;
        let purchase_unit = PaypalPurchaseUnitRequest {
            reference_id: None, // Could use data.merchant_order_reference_id or similar if present
            amount: PaypalMoney {
                currency_code: currency,
                value: StringMajorUnit::new(amount);
            },
        };
        // Card details
        let card = match &data.payment_method_data {
            PaymentMethodData::Card(card) => {
                let expiry = format!("{}-{}", card.card_exp_year.peek(), card.card_exp_month.peek());
                Some(PaypalPaymentSourceCard {
                    card: PaypalCardDetails {
                        number: card.card_number.to_string().into(),
                        expiry: expiry.into(),
                        security_code: Some(card.card_cvc.clone()),
                        name: None, // No card_holder_name field
                    },
                })
            },
            _ => None,
        };
        let payment_source = card.map(|c| PaypalPaymentSource { card: Some(c) });
        let application_context = Some(PaypalApplicationContext {
            return_url: data.router_return_url.clone(),
            cancel_url: data.webhook_url.clone(), // Not always present, but Paypal expects it
        });
        Ok(PaypalPaymentRequest {
